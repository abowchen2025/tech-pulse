...
每類找1-2篇當天或近3天內的新聞，只挑當下最重要的，不用求全。{topic_avoid_note}
...
另外搜尋今天實際的 GitHub Trending 頁面（https://github.com/trending，可加上 since=daily 或 since=weekly 參數），找1-2個真正當前上榜、跟以上四大主題相關的開源專案即可，找到明顯符合的就停止，不用窮舉比較多個候選。務必確認選中的專案「近期真的有新版本發布、功能更新、或star數在近期明顯增長」，不要只憑專案知名度或印象挑選。如果完全找不到任何真正當前上榜且相關的專案，這個段落可以整段省略，不要為了湊數勉強放進不符合條件的專案。{trend_avoid_note}
